    ...
    >
      ...
      >
        ...
        <div className="sticky top-0 bg-[#111] border-b border-white/10 px-6 py-4 flex justify-between items-center z-10">
          <h2 className="text-xs font-mono uppercase tracking-[0.2em] text-lime-400">
        ...
        <div className="px-6 py-5 space-y-5">
          ...
          {details.physicalDescription && (
            <div>
              <p className="text-xs font-mono text-white/30 mb-1.5">Description</p>
              <p className="text-sm text-white/70 leading-relaxed">{details.physicalDescription}</p>
            </div>
          )}
          ...
          {details.physicalImageUrls.length > 0 && (
            <div>
              <p className="text-xs font-mono text-white/30 mb-2">Product Photos</p>
          ...
          <div className="space-y-2">
            <p className="text-xs font-mono text-white/30 mb-1.5">Price Includes</p>
            <div className="flex items-center gap-2">
              <span className={`text-xs ${details.priceIncludesTax ? 'text-lime-400' : 'text-white/20'}`}>
                {details.priceIncludesTax ? '✓' : '✕'}
              </span>
              <span className="text-xs text-white/50">All applicable taxes</span>
            </div>
            <div className="flex items-center gap-2">
              <span className={`text-xs ${details.priceIncludesPacking ? 'text-lime-400' : 'text-white/20'}`}>
                {details.priceIncludesPacking ? '✓' : '✕'}
              </span>
              <span className="text-xs text-white/50">Packing for shipment</span>
            </div>
          </div>

          {/* Shipping */}
          <div>
            <p className="text-xs font-mono text-white/30 mb-1.5">Shipping</p>
            {details.shippingType === 'included' ? (
              <div>
                <p className="text-xs text-white/50 mb-1">Included in price for:</p>
                <div className="flex flex-wrap gap-1.5">
                  {(details.shippingIncludedRegions ?? []).map((region) => (
                    <span
                      key={region}
                      className="px-2 py-0.5 text-[10px] font-mono border border-lime-400/20 text-lime-400/60"
                    >
                      {region}
                    </span>
                  ))}
          ...
          {details.collectionInPerson && (
            <div>
              <p className="text-xs font-mono text-white/30 mb-1.5">Collection in Person</p>
              <p className="text-xs text-white/50">{details.collectionInPerson}</p>
            </div>
          )}
          ...
          {details.ecommerceUrl && (
            <div>
              <p className="text-xs font-mono text-white/30 mb-1.5">Also Available At</p>
              <a
                href={details.ecommerceUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="text-xs text-white/40 hover:text-white/70 transition-colors font-mono underline"
              >
