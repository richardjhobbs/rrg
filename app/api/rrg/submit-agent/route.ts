 ...
 * needing multipart/form-data. Accepts an image_url instead of
 * a file upload — the server fetches the image itself.
 *
 * Body (JSON):
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!image_url || typeof image_url !== 'string') {
      return NextResponse.json(
        { error: 'image_url is required — provide a publicly accessible JPEG URL' },
        { status: 400 }
      );
    }
    ...
    const isJpeg =
      ...
      // Also accept if URL ends in .jpg/.jpeg and content-type is octet-stream
